...
services:
  subversion:
    ...
    image: ghcr.io/intechcore/subversion-ldap-httpd:1.14.5-4
    ports:
      - "8080:8080"
    volumes:
      - ./repos:/svn/repos
...
rebuild reason or the Subversion update, then the `CHANGELOG.md` entries added since the previous
release. A table lists Subversion and the base image with its digest. The commits follow in a
